# OBSS SAHI Tool
# Code written by AnNT, 2023 and adapted by Hamzalopode 2024.
...
class Yolov9DetectionModel(DetectionModel):
    ...
    def load_model(self):
        ...
        try:
            model = load_model(self.model_path, device=self.device)
            model.to(self.device)
            self.set_model(model)
        except Exception as e:
            raise TypeError("model_path is not a valid yolov9 model path: ", e)
    ...
    def perform_inference(self, image: np.ndarray):
        ...
        if self.image_size is not None:  # ADDED IMAGE SIZE OPTION FOR YOLOV9 MODELS:
            prediction_result = self.model(
                image[:, :, ::-1], imgsz=self.image_size, verbose=False, device=self.device
            )  # YOLOv9 expects numpy arrays to have BGR
        else:
            prediction_result = self.model(
                image[:, :, ::-1]
            )  # YOLOv9 expects numpy arrays to have BGR
...
def load_model(
    ...
    try:
        model = DetectMultiBackend(
            model_path, device=device, fuse=autoshape
        )  # detection model
        if autoshape:
            try:
