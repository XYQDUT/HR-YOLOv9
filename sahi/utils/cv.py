...
def exif_transpose(image):
    ...
    参数:
        image (PIL.Image.Image): 要转置的图像。

    返回:
        PIL.Image.Image: 转置后的图像。
    """
    try:
        exif = image._getexif()
        if exif:
...
def read_image_as_pil(image, exif_fix=False):
    """
    加载图像为PIL.Image.Image。

    参数:
        image (Union[Image.Image, str, np.ndarray, torch.Tensor]): 要加载的图像。可以是图像路径或URL（str），
            numpy图像（np.ndarray），PIL.Image对象或张量（torch.Tensor）。
        exif_fix (bool, optional): 是否对图像应用EXIF修正。默认为False。

    返回:
        PIL.Image.Image: 加载为PIL.Image对象的图像。
    """
    # 允许加载超大图像
    ...
    if isinstance(image, Image.Image):
        ...
        try:
            ...
            if image.startswith("http"):
                image_pil = Image.open(requests.get(image, stream=True).raw).convert("RGB")
            else:
            ...
            try:
                ...
                if len(image_sk.shape) == 2:  # 黑白图像
                    image_pil = Image.fromarray(image_sk, mode="L")
                elif image_sk.shape[2] == 4:  # RGBA
                    image_pil = Image.fromarray(image_sk, mode="RGBA")
                elif image_sk.shape[2] == 3:  # RGB
                    image_pil = Image.fromarray(image_sk, mode="RGB")
                else:
                    raise TypeError(f"不支持形状为: {image_sk.shape[2]} 的图像。")
            except ImportError:
                raise ImportError("请运行 'pip install -U scikit-image imagecodecs' 以处理大图像。")
            except Exception as e:
                raise e
    elif isinstance(image, np.ndarray):
        if image.shape[0] < 5:  # CHW格式图像
            image = image.transpose((1, 2, 0))
        image_pil = Image.fromarray(image)
    elif isinstance(image, torch.Tensor):
        image_pil = tensor_to_image(image)
    else:
        raise TypeError("无法使用 'Pillow' 加载图像，请使用 'Image.open()' 方法。")
